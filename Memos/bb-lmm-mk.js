...
`;Utils.loadCssCode(allCSS);const AppState={limit:bbMemo.limit,memos:bbMemo.memos,memosOpenId:null,mePage:1,offset:0,nextLength:0,nextDom:"",apiV1:"",bbDom:bbMemo.domId?document.querySelector(bbMemo.domId):null,isLoading:false,tageFilter:"",emactionApi:bbMemo.emactionApi};const load='<div class="bb-load"><button class="load-btn button-load">加载中……</button></div>';const loading=`<div class="loader"><svg class="circular" viewBox="25 25 50 50"><circle class="path" cx="50" cy="50" r="20" fill="none" stroke-width="2" stroke-miterlimit="10"/></svg></div>`;if(AppState.bbDom){Utils.safeExecute(()=>fetchStatus(),"初始化失败").catch(e=>{console.error("应用启动失败:",e);AppState.bbDom.innerHTML='<div class="error">加载失败，请刷新页面重试</div>'})}async function fetchStatus(){let e=AppState.memos+"api/v1";let t=await fetch(e);if(t.ok||t.status===404){AppState.apiV1="v1/"}let o=Utils.getQueryVariable("memo")||"";if(o){getMemoOne(o)}else{newApiV1(AppState.apiV1)}}function getMemoOne(e){let t=`<iframe style="width:100%;height:100vh;" src="${e}" frameBorder="0"></iframe>`;let o=document.querySelector(".content")||document.querySelector(bbMemo.domId);o.innerHTML=t}function newApiV1(e){getFirstList(e);meNums(e);AppState.bbDom.innerHTML=loading}function bindLoadMoreButton(o){setTimeout(()=>{const e=document.querySelector("button.button-load");if(e&&!e.hasAttribute("data-bound")){e.setAttribute("data-bound","true");const t=function(e){e.preventDefault();e.stopPropagation();const t=e.target;t.textContent="";t.classList.add("loading");t.disabled=true;updateHTMl(AppState.nextDom);if(AppState.nextDom.memos.length===0||AppState.nextDom.memos.length<AppState.limit){t.classList.remove("loading");t.textContent="没有更多了";t.disabled=true}else{getNextList(o||AppState.apiV1)}};e.addEventListener("click",t);e._clickHandler=t}},100)}async function getFirstList(t){try{AppState.bbDom.insertAdjacentHTML("afterend",load);bindLoadMoreButton(t);let e=AppState.memos+"api/"+t+"memos?creatorId="+bbMemo.creatorId+"&filter=creator_id == 1&pageSize="+AppState.limit;const o=await fetch(e);if(!o.ok){throw new Error(`HTTP error! status: ${o.status}`)}const r=await o.json();updateHTMl(r);AppState.offset=r.nextPageToken;if(AppState.offset===""||!r.memos||r.memos.length===0){const a=document.querySelector("button.button-load");a.textContent="没有更多了";a.disabled=true;return}AppState.mePage++;getNextList(t)}catch(e){console.error("获取数据失败:",e);AppState.bbDom.innerHTML='<div class="error">加载失败，请刷新页面重试</div>'}}async function getNextList(t){try{if(AppState.isLoading)return;if(AppState.offset===""){const a=document.querySelector("button.button-load");a.textContent="没有更多了";a.disabled=true;return}AppState.isLoading=true;let e=AppState.memos+"api/"+t+"memos?creatorId="+bbMemo.creatorId+"&pageSize="+AppState.limit+"&pageToken="+AppState.offset;if(AppState.tageFilter){e=e+'&filter=tag in ["'+AppState.tageFilter+'"]'}const o=await fetch(e);if(!o.ok){throw new Error(`HTTP error! status: ${o.status}`)}const r=await o.json();AppState.nextDom=r;AppState.mePage++;AppState.offset=r.nextPageToken}catch(e){console.error("预加载下一页失败:",e)}finally{AppState.isLoading=false}}function meNums(e){let t=document.querySelector(".bb-load")}async function updateHTMl(e){if(!e||!e.memos){console.error("数据格式错误");return}let p="",r="";const b=/#([^#\s!.,;:?"'()]+)(?= )/g,f=/\!\[(.*?)\]\((.*?)\)/g,u=/\[(.*?)\]\((.*?)\)/g,g=/<a.*?href="https:\/\/www\.bilibili\.com\/video\/((av[\d]{1,10})|(BV([\w]{10})))\/?".*?>.*<\/a>/g,h=/<a.*?href="https:\/\/music\.163\.com\/.*id=([0-9]+)".*?>.*<\/a>/g,v=/<a.*?href="https\:\/\/y\.qq\.com\/.*(\/[0-9a-zA-Z]+)(\.html)?".*?>.*?<\/a>/g,y=/<a.*?href="https:\/\/v\.qq\.com\/.*\/([a-z|A-Z|0-9]+)\.html".*?>.*<\/a>/g,x=/<a.*?href="https:\/\/v\.youku\.com\/.*\/id_([a-z|A-Z|0-9|==]+)\.html".*?>.*<\/a>/g,w=/<a.*?href="https:\/\/www\.youtube\.com\/watch\?v\=([a-z|A-Z|0-9]{11})\".*?>.*<\/a>/g;if(typeof marked==="undefined"){console.error("marked库未加载");return}marked.setOptions({breaks:false,smartypants:false,langPrefix:"language-",headerIds:false,mangle:false});const S=e.memos;for(let m=0;m<S.length;m++){let e=S[m].name;let t=AppState.memos+e;let o=S[m].content+" ";let r="";r+=o.replace(b,"").replace(f,"").replace(u,'<a class="primary" href="$2" target="_blank">$1</a>');let a=o.match(b);let i="";if(a){i=a.map(e=>{return`<span class='tag-span' onclick='getTypeOfMemos(this)'>${e}</span> `}).join("");r=i+r.trim()}r=marked.parse(r).replace(g,"<div class='video-wrapper'><iframe src='//www.bilibili.com/blackboard/html5mobileplayer.html?bvid=$1&as_wide=1&high_quality=1&danmaku=0' scrolling='no' border='0' frameborder='no' framespacing='0' allowfullscreen='true'></iframe></div>").replace(h,"<meting-js auto='https://music.163.com/#/song?id=$1'></meting-js>").replace(v,"<meting-js auto='https://y.qq.com/n/yqq/song$1.html'></meting-js>").replace(y,"<div class='video-wrapper'><iframe src='//v.qq.com/iframe/player.html?vid=$1' allowFullScreen='true' frameborder='no'></iframe></div>").replace(x,"<div class='video-wrapper'><iframe src='https://player.youku.com/embed/$1' frameborder=0 'allowfullscreen'></iframe></div>").replace(w,"<div class='video-wrapper'><iframe src='https://www.youtube.com/embed/$1' title='YouTube video player' frameborder='0' allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture' allowfullscreen title='YouTube Video'></iframe></div>");let n=S[m].content.match(f)||"",l="";if(n){let t=n.length,o="";if(t!==1){let e=" grid grid-"+t}n.forEach(e=>{let t=e.replace(/!\[.*?\]\((.*?)\)/g,"$1");o+=`<figure class="gallery-thumbnail"><img class="img thumbnail-image" loading="lazy" decoding="async" src="${t}"/></figure>`});r+=`<div class="resimg${l}">${o}</div>`}if(S[m].resources&&S[m].resources.length>0){let a=S[m].resources;let i="",n="",l=0;for(let r=0;r<a.length;r++){let e=a[r].type.slice(0,5);let t=a[r].externalLink;let o=t?t:AppState.memos+"file/"+(a[r].publicId||a[r].name)+"/"+a[r].filename+"?thumbnail=1";if(e=="image"){i+=`<figure class="gallery-thumbnail"><img class="img thumbnail-image" src="${o}"/></figure>`;l=l+1}else if(e=="video"){i+=`<div class="video-wrapper"><video controls><source src="${o}" type="video/mp4"></video></div>`}else{n+=`<a target="_blank" rel="noreferrer" href="${o}">${a[r].name}</a>`}}if(i){let e="";e="grid grid-"+l;r+=`<div class="resimg ${e}">${i}</div>`}if(n){r+=`<p class="bb-source">${n}</p>`}}let s=AppState.memos.replace(/https\:\/\/(.*\.)?(.*)\..*/,"id-$2-");let d=`<emoji-reaction theme="system" class="reaction" endpoint="${AppState.emactionApi}" reacttargetid="${s+"memo-"+e}" style="line-height:normal;display:inline-flex;"></emoji-reaction>`;let c="";p+=`<div data-id="memo-${e}" class="memo-item">
        <div class="bb-item">
